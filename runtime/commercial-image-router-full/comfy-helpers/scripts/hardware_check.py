...
def detect_nvidia() -> dict | None:
    ...
    # Copy the selected dict before attaching all_gpus. Otherwise all_gpus
    # contains `best` itself and json.dumps(report) hits a circular reference.
...
def detect_rocm() -> dict | None:
    ...
    if out.strip().startswith("{"):
        try:
            ...
            if cards:
                # Copy the selected dict before attaching all_gpus. Otherwise
                # all_gpus contains `best` itself and json.dumps(report) hits a
                # circular reference.
