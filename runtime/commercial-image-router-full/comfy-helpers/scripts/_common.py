...
def unwrap_workflow(payload: Any) -> dict:
    ...
    if isinstance(payload, dict) and is_api_format(payload):
        return {k: v for k, v in payload.items() if isinstance(v, dict) and "class_type" in v}
    ...
    if isinstance(payload, dict) and "prompt" in payload and is_api_format(payload["prompt"]):
        return {k: v for k, v in payload["prompt"].items() if isinstance(v, dict) and "class_type" in v}
    # Editor format
